	...
	public function index($setting) {
		...
		if (count($products) < 4)
		{
			...
			foreach ($query->rows as $result) { 		
				array_push($products,$result['product_id']);
        ...
        if (isset($this->request->get['route']) && $this->request->get['route'] == 'product/product') {
            ...
            setcookie('viewed', implode(',',$products), time() + 60 * 60 * 24 * 120, '/', $this->request->server['HTTP_HOST']);
        }
			...
			if ($product_info) {
				if ($product_info['image']) {
					...
					$image = $this->model_tool_image->resize('placeholder.png', $setting['width'], $setting['height']);
						...
						if ((($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) && $product_info['quantity'] > 0) {
